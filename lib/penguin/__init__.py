...
def decrypt_stream(server: str, data: bytes, key0: int, key1: int, key2: int, isAssets: bool = False):
    if isAssets == False:
        key0 = key0 ^ CONST_KEYS[server][0]
        key1 = key1 ^ CONST_KEYS[server][1]
        key2 = key2 ^ CONST_KEYS[server][2]
    countView = 5
    out = bytearray()
    for position in range(0, data.__len__()):
        temp = int.from_bytes(
            data[position:position+1], byteorder="big") ^ (((key1 ^ key0 ^ key2) >> 24) & 0xff)
